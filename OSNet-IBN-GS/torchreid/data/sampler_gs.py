...
AVAI_SAMPLERS = [
    ...
    'RandomDomainSampler', 'RandomDatasetSampler', 'GraphSampler'
]

@torch.no_grad()
def calc_distance(model, dataset=None, transform=None, dist_metric='euclidean', normalize_feature=False):
    data_loader = DataLoader(
        Preprocessor(dataset, root=None, transform=transform),
    ...
    distmat = metrics.compute_distance_matrix(features, features, dist_metric)
...
class GraphSampler(Sampler):
    def __init__(self, data_source, batch_size, num_instance, model=None, transform=None, verbose=True):
        if batch_size < num_instance:
            raise ValueError(
                'batch_size={} must be no less '
                'than num_instances={}'.format(batch_size, num_instance)
            )
    ...
    def graph_index(self):                
        ...
        start = time.time()
        dist = calc_distance(self.model, dataset, self.transform)
...
def build_train_sampler(
    ...
    **kwargs
    ...
    if train_sampler == 'GraphSampler':
        sampler = GraphSampler(data_source, batch_size, num_instances, **kwargs)
